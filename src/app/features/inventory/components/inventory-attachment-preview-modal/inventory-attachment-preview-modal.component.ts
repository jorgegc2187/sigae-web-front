...
import { AssetAttachmentSummary } from '../../models/inventory.model';
import { AssetsService } from '../../services/assets.service';

@Component({
  selector: 'app-inventory-attachment-preview-modal',
  imports: [ActionButtonComponent],
  templateUrl: './inventory-attachment-preview-modal.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class InventoryAttachmentPreviewModalComponent implements OnDestroy {
  readonly open = input(false);
  readonly attachment = input<AssetAttachmentSummary | null>(null);
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (this.loadedAttachmentId() === attachment.id) {
        return;
      }
  ...
  private async loadAttachment(attachment: AssetAttachmentSummary): Promise<void> {
    this.revokeObjectUrl();
  ...
  private async resolveBlob(attachment: AssetAttachmentSummary): Promise<Blob> {
  ...
  private async fetchBlob(attachment: AssetAttachmentSummary): Promise<Blob> {
  ...
  private isImage(attachment: AssetAttachmentSummary): boolean {
    return attachment.mimeType.startsWith('image/');
  }

  private isPdf(attachment: AssetAttachmentSummary): boolean {
